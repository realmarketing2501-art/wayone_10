...
import { Layers, ArrowUpDown, Gift, Save } from 'lucide-react';
...
export default function CompensationTab() {
  ...
  const { data: levels = DEFAULT_LEVELS, isLoading: levelsLoading } = useAdminSetting<LevelConfig[]>('level_config', DEFAULT_LEVELS);
  const { data: withdrawals = DEFAULT_WITHDRAWALS, isLoading: withdrawalsLoading } = useAdminSetting<WithdrawalConfig[]>('withdrawal_config', DEFAULT_WITHDRAWALS);
